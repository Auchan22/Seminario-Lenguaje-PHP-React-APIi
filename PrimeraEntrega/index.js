...
btnSubmitPedido.addEventListener("click", (e) => {
    if(!Number(selectItem.value) || !Number(selectMesa.value)){
        e.preventDefault();
        if(!Number(selectItem.value) && !Number(selectMesa.value)) {alert("Se debe seleccionar un Itém y una mesa"); return;}
        if(!Number(selectItem.value) && Number(selectMesa.value)) {alert("Se debe seleccionar un Itém"); return;}
        if(Number(selectItem.value) && !Number(selectMesa.value)) {alert("Se debe seleccionar una mesa"); return;}
    }
